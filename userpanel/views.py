...
def user_blog_list(request):
    # Display all blogs authored by the logged-in user, ensure non-blocked users' blogs are visible
    blogs = Blog.objects.filter(author=request.user, status='published').order_by('-created_at')
    return render(request, 'userpanel/blog_list.html', {'Blog': blogs})
